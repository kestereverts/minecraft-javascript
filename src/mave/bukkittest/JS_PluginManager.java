...
public class JS_PluginManager extends ScriptableObject
{
	...
	public static void jsFunction_registerEvent(Context cx, Scriptable thisObj, Object[] args, Function funObj)
	{
		...
		{
			reg.m_listener = new BukkitTestBlockListener(reg);
		}
		else if (category.equals("player"))
		{
			reg.m_listener = new BukkitTestPlayerListener(reg);
		}
		else if (category.equals("entity"))
		{
			reg.m_listener = new BukkitTestEntityListener(reg);
		}
		else
		{
		...
		reg.m_script = BukkitTest.m_currentScript;
		...
		caller.pluginManager.registerEvent(event, reg.m_listener, priority, BukkitTest.m_singleton);
	}
}
